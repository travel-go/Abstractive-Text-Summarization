...
class LSTMDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            self.attention_layers.append(AttentionLayer(encoder_output_units, hidden_size) )
        ...
        self.re_fc3= Linear(out_embed_dim, num_embeddings, dropout=dropout_out)


    def forward(self, prev_output_tokens, encoder_out_dict, model_type=None,incremental_state=None):
        ...
        attn_scores = x.data.new(srclen, seqlen, bsz, self.num_layers).zero_()
        outs = []
        complement_s = []
        for j in range(seqlen):
            ...
            for i, (rnn, attention) in enumerate(zip(self.layers, self.attention_layers)):
            # for i, rnn in enumerate(self.layers):
                ...
                tmp_input, attn_scores[:, j, :, i] = attention(input, encoder_outs, encoder_padding_mask)
            ...
            #     out = hidden
                if i==2:
                    single_attention_ = attn_scores[:, j, :,i].clone()
                    single_attention_ = single_attention_.transpose(0, 1).unsqueeze(1)
                    corresponding_hidden = encoder_outs.transpose(0, 1)
                    ...
                    complement_attention = F.dropout(complement_attention, p=self.dropout_out, training=self.training)
                    complement_s.append(complement_attention)
        ...
        y = torch.cat(complement_s, dim=1)
        y = self.re_fc3(y)
