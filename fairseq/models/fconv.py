...
class FConvDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.re_fc2 = Linear(in_channels,out_embed_dim)
        self.re_fc3 = Linear(out_embed_dim, num_embeddings, dropout=dropout)

    def forward(self, prev_output_tokens, encoder_out_dict=None,model_type=None, incremental_state=None):
        ...
        for num,(proj, conv, attention, res_layer) in enumerate(zip(self.projections, self.convolutions, self.attention,
                                                    self.residuals)):
            ...
            if attention is not None:
                ...
                if num == 1:
                    single_attention = attn_scores
                    corresponding_hidden = encoder_b
        ...
        # constrastive
        single_attention_ = single_attention.clone()
        ...
        complement_attention = self.re_fc2(complement_attention)
        complement_attention = F.dropout(complement_attention, p=self.dropout, training=self.training)
        contrastive_attn = self.re_fc3(complement_attention)
        ...
        return x, avg_attn_scores, contrastive_attn
...
def fconv_iwslt_de_en(args):
    ...
    args.encoder_layers = getattr(args, 'encoder_layers', '[(256, 3)] * 6')
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 256)
    args.decoder_layers = getattr(args, 'decoder_layers', '[(256, 3)] * 6')
    args.decoder_out_embed_dim = getattr(args, 'decoder_out_embed_dim', 256)
    base_architecture(args)
