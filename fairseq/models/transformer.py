...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, left_pad=False, final_norm=True):
        ...
        # self.re_embed_out = nn.Parameter(torch.Tensor(len(dictionary), output_embed_dim))
        # nn.init.normal_(self.re_embed_out, mean=0, std=output_embed_dim ** -0.5)


    # def forward(self, prev_output_tokens, writer, encoder_out=None,model_type=None,incremental_state=None):
    def forward(self, prev_output_tokens, encoder_out=None,model_type=None,incremental_state=None):
        """
        Args:
        ...
        # decoder layers
        single_attention = None
        corresponding_hidden = None


        for num,layer in enumerate(self.layers):
            x, attn,single_weight,v_hidden = layer(
                x,
            ...
            if num == 2:
                single_attention = single_weight
                corresponding_hidden = v_hidden
        ...
        if model_type is None:
            ...
            for i in range(1):
                ...
                single_attention_.masked_fill_(single_attention_.eq(max_),float("-inf"))

            single_attention_.masked_fill_(single_attention_.eq(0),float("-inf"))
            tmp_attn = single_attention_.eq(float("-inf"))
            tmp_attn_max = torch.sum(tmp_attn, dim=-1)
            single_attention_ = F.softmax(single_attention_, dim=-1)
            ...
            single_attention_ = single_attention_.view(bsz, tgt_len, src_len)
            ...
            bsz, tgt_len ,src_len = single_attention_.size()
            index_list = []
            index_tensor = torch.arange(0, src_len).unsqueeze(0).repeat(bsz,tgt_len,1)
            for i in range(10):
                noise = torch.randn(bsz, tgt_len, src_len)* src_len
                index_list.append(index_tensor.add_(noise).sort()[1])
            if encoder_out['encoder_padding_mask'] is None:
                tmp_x =None
                for i in range(10):
                    if tmp_x is None:
                        tmp_x = torch.gather(single_attention_, -1, index_list[i].cuda())
                    else:
                        tmp_x = tmp_x+ torch.gather(single_attention_, -1, index_list[i].cuda())
                tmp_x = tmp_x / 10
                single_attention_ = tmp_x
                single_attention_ = F.softmax(single_attention_, dim=-1)
            else:
                mask_ = single_attention_.eq(0)
                tmp_x =None
                for i in range(10):
                    if tmp_x is None:
                        tmp_x = torch.gather(single_attention_, -1, index_list[i].cuda())
                ...
                tmp_x = tmp_x / 10
                single_attention_ = tmp_x
                single_attention_.masked_fill_(mask_, float('-inf'))
                single_attention_ = F.softmax(single_attention_, dim=-1)
            single_attention_ = single_attention_.view(bsz, tgt_len, src_len)
        elif model_type =="max2inf":
        # max2inf
            single_attention_ = single_attention[:, 1, :, :].clone()
            corresponding_hidden = corresponding_hidden[:, 1, :, :]
            bsz, tgt_len ,src_len = single_attention_.size()
            single_attention_ = single_attention_.view(-1,src_len)
            max_ = torch.max(single_attention_, 1)[0].unsqueeze(1).repeat(1,src_len)
            single_attention_.masked_fill_(single_attention_.eq(max_),float("-inf"))
        ...
        complement_attention = torch.bmm(single_attention_, corresponding_hidden)
            # print(complement_attention.requires_grad)
        complement_attention = self.attn_out(complement_attention)
        complement_attention = F.dropout(complement_attention, p=self.dropout, training=self.training)
        complement_attention = self.re_layer_norm_1(complement_attention)
        re_residual = complement_attention
        complement_attention = F.relu(self.re_fc_1(complement_attention))
        complement_attention = self.re_fc_2(complement_attention)
        complement_attention = F.dropout(complement_attention, p=self.dropout, training=self.training)
        complement_attention = re_residual + complement_attention
        complement_attention = self.re_layer_norm_2(complement_attention)
        complement_attention = F.linear(complement_attention, self.re_embed_out)
        ...
        return x, attn, complement_attention
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask):
        ...
        x, _, _, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state):
        ...
        if self.encoder_attn is not None:
            ...
            x, attn, single_weight,corresponding_hidden = self.encoder_attn(
                query=x,
                key=encoder_out,
                value=encoder_out,
        ...
        return x, attn, single_weight,corresponding_hidden
