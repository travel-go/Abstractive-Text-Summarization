...
class SequenceGenerator(object):
    def __init__(
        ...
        elif diverse_beam_groups > 0:#self.diverse_beam_groups=1
    ...
    def _generate(self, src_tokens, src_lengths, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        model_type = []
        model_type.append("max2inf")
        model_type.append("random")



        #incremental_states先存了模型的结构
        for model in self.models:
            ...
            #encoder_out["encoder_out"].size() src_len*(beam_size*src_num)*dim
            #encoder_out["encoder_padding_mask"].size() (beam_size*src_num)*src_len
            encoder_outs.append(encoder_out)

        # initialize buffers
        scores = src_tokens.data.new(bsz * beam_size, maxlen + 1).float().fill_(0)
        #scores里面应该是存的是所有句子的得分
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores,tmp_decoder = self._decode(tokens[:, :step + 1], encoder_outs, model_type, incremental_states)
            ...
            #首先cand_beams是每个词beam_search中的第几位，然后bbsz_offsets是每个tgt_num中的对应位置，两者相加之后得到最终的一个每次tgt_num中的位置
            #cand_beams不仅表示了这个词的beam_search还有前一个词的beam_search
            ...
            if step >= self.minlen:
                ...
                #挑选出eos的部分，然后得到对应位置的0或者1，是eos则为1，不是则为0
                torch.masked_select(
            ...
            #通过index挑选tokens在维度为0还是维度为1的部分
            #这里是根据idx（混合了beam的编号）挑选出相对应的词
            #这里为什么tokens_buf回合原来的tokens发生变化
            #因为beam search会挑选出在第二次也会挑选出前10个最好的，但是可能第二次挑选最好的时候前一个词或者前几个词的索引有所改变
            torch.index_select(
                tokens[:, :step + 1], dim=0, index=active_bbsz_idx,
                out=tokens_buf[:, :step + 1],
            )
            #gather就是根据索引找对应位置上面的数
            torch.gather(
    ...
    def _decode(self, tokens, encoder_outs,model_types, incremental_states):
        ...
        for model, encoder_out,model_type in zip(self.models, encoder_outs,model_types):
            probs, attn,tmp_decoder = self._decode_multi_one(tokens, model, encoder_out,model_type, incremental_states, log_probs=True)
        ...
        return avg_probs, avg_attn,tmp_decoder
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs):
        ...
        re_probs = model.get_re_normalized_probs(decoder_out, log_probs=log_probs)


        probs=probs+0.2*re_probs
        # probs=probs
        # probs=re_probs

        return probs, attn, tmp_decoder

    def _decode_multi_one(self, tokens, model, encoder_out, model_type,incremental_states, log_probs):
        with torch.no_grad():
            if incremental_states[model] is not None:
                decoder_out = list(model.decoder(tokens, encoder_out,model_type, incremental_state=incremental_states[model]))
            else:
                decoder_out = list(model.decoder(tokens, encoder_out,model_type))
